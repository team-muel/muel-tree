...
**결정 필요 (백엔드 착수 전):**
- (a) 범위: 테이블 목록을 **같은 음성 채널 안**에서 고르나(인스턴스 모델 유지, 채널당 복수 매치 허용), 아니면 **서버(길드) 전체 로비 브라우저**인가? 후자는 Activity 진입점·권한·정리(orphan 매치 GC) 설계가 더 커진다.
- (b) 정리: 빈/방치 테이블 lifecycle(자동 소멸 시점).

→ 이 결정 전까지 B5 는 착수하지 않는다. 결정되면 muel-bot 엣지(`match-list`+create-new) + 마이그레이션 → 프론트 목록 UI 순으로 두 PR.
