...
export const GOMDORI_ROLES: Record<string, GomdoriRoleMeta> = {
  ...
  besto: {
    ...
    roster: "demon",
    faction: "demon",
    reveal: "히든 포지션의 악마. 미발동 강화 스택으로 멀티타깃 처치를 모으고, 변신으로 조사와 투표가치를 흔들며, 누명씌우기로 강화를 회수합니다.",
    passive: "두 번째 자아: 솔(천사 판정·투표가치 1 고정) ↔ 하베스토(악마 판정·투표가치 3+강화×2 고정) 토글. 사탄의 마(-1) 누적과 무관하게 행사 투표가치가 절대값으로 고정됩니다. 미발동 밤마다 히든 강화 +1(상한 2). 배후(조력자-베스토 효과 반전 통지)는 후속.",
    abilitySummary: "히든 포지션(2회 제한): 강화 스택에 따라 1+강화 명을 동시 처치하고 발동 시 중첩 리셋. 변신(두 번째 자아): 조사 결과와 행사 투표가치(솔 1 / 하베스토 3·5·7)를 토글. 누명씌우기(짝숫날 제외): 대상에 표식을 남기고, 그 대상이 그 밤 탈락하면 히든 강화 +1.",
    demonTeam: true,
    night: { actionType: "besto_hidden", label: "히든 포지션", prompt: "조력자와 상의하여 오늘 밤 처치할 대상을 고르세요. 미발동 강화(최대 2)만큼 추가 대상을 동시 지정할 수 있습니다. 발동 시 강화는 초기화되고, 사용 횟수는 2회입니다.", excludeSelf: true, kind: "kill", maxTargets: 1, maxTargetsSelfBonus: true },
    extraNights: [
      { actionType: "besto_shift", label: "변신", prompt: "변신 — 솔(조사 시 천사·투표가치 1 고정) ↔ 하베스토(악마 판정·투표가치 3+강화×2 고정)로 전환합니다.", self: true },
      { actionType: "besto_frameup", label: "누명씌우기", prompt: "누명을 씌울 대상을 고르세요. 그 대상이 그 밤 탈락하면 히든 강화가 +1 됩니다(베스토·조력자 제외). 짝숫날에는 발동되지 않습니다.", excludeSelf: true },
    ],
  },
...
export const GOMDORI_ORIGINAL_ABILITIES: Record<string, GomdoriOriginalAbility[]> = {
  ...
  besto: [
    { kind: "패시브", name: "두 번째 자아", text: "밤마다 솔(천사 판정·투표가치 1 고정)과 하베스토(악마 판정·투표가치 3+강화×2 고정) 사이를 토글합니다. 사탄의 마(-1) 누적과 무관하게 행사 투표가치는 절대값으로 고정됩니다.", actionType: "besto_shift", status: "live" },
    { kind: "특수 패시브", name: "배후", text: "조력자와 영혼을 교체해 베스토·조력자 대상 능력 효과가 반대로 통지됩니다. reveal 파이프라인 재설계 필요라 후속 확장 대상입니다.", status: "planned" },
    { kind: "능력", name: "히든 포지션", text: "미발동 밤마다 강화 +1(상한 2)을 모아 1+강화 명을 동시에 처치합니다. 발동 시 중첩이 초기화되고 사용 횟수는 2회입니다. 멘션 기반(아침 토론 텍스트 파싱)은 후속이며 현재는 밤 지목으로 근사합니다.", actionType: "besto_hidden", status: "live" },
    { kind: "능력2", name: "누명씌우기", text: "대상에 표식을 남깁니다. 그 대상이 그 밤 탈락하면 베스토의 히든 강화가 +1 됩니다(베스토·조력자 제외, 짝숫날 발동 불가).", actionType: "besto_frameup", status: "live" },
  ],
  gain: [
